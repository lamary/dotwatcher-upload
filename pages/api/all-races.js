...
export default async function handle(req, res) {
  try {
    ...
    const { rows: races } = await db.query(`SELECT name, array_agg(year) as years, array_agg(length) as lengths, array_agg(id) as ids, array_agg(slug) as slugs FROM races GROUP BY name;`)
